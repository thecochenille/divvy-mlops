...
def transform_df_target(df):
    ...
    rentals = df.groupby(['start_station_id', 'started_hour', 'started_day_of_week']).size().reset_index(name='average_rentals')
    returns = df.groupby(['end_station_id', 'ended_hour', 'ended_day_of_week']).size().reset_index(name='average_returns')

    usage_df = pd.merge(rentals, returns, left_on=['start_station_id', 'started_hour', 'started_day_of_week'], right_on=['end_station_id', 'ended_hour', 'ended_day_of_week'], how='outer')
...
def extract_station_info(df):
  '''
  '''
  df['station_id'] = df.apply(lambda row: row['end_station_id'] if not pd.isna(row['end_station_id']) else row['start_station_id'], axis=1)
  df['hour'] = df.apply(lambda row: row['ended_hour'] if not pd.isna(row['end_station_id']) else row['started_hour'], axis=1)
  df['day_of_week'] = df.apply(lambda row: row['ended_day_of_week'] if not pd.isna(row['end_station_id']) else row['started_day_of_week'], axis=1)

  return df[['net_usage', 'station_id', 'hour', 'day_of_week']] #this is the final dataset I need
...
def main():
    while True:
        ...
        filename_output = f'{path_output}/{year}{month}-usage-stationID.parquet'
